...
public class StudentServletController extends HttpServlet {
	...
	private void dispatchRequest(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (partialUrl[0].equalsIgnoreCase("changePassword")) {
			if (method.equalsIgnoreCase("get")) {
				request.getRequestDispatcher("/pages/student/changePassword.jsp").forward(request,response);
			...
			if (partialUrl[1].equalsIgnoreCase("create") && method.equalsIgnoreCase("get")) {
				...
				sendContactUs(request, response);
			}
		}
	}

	private void sendContactUs(HttpServletRequest request, HttpServletResponse response)
	...
	private void changePassword(HttpServletRequest request, HttpServletResponse response)
		...
		User student = userDao.findUserByUsernameAndPassword(user.getUsername(), oldPass);
		if (student != null) {
			student.setPassword(newPass);
			userDao.updateUser(student);
			session.setAttribute("currentUser", student);
			request.setAttribute("msg", "Update password successfully");
